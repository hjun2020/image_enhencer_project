...
extern void test2(char *cfgfile, char *weightfile);
...
extern void resize_image_save(char *filename, int w, int h, char *outfile);
...
int main(int argc, char **argv)
{
    ...
    //     test_resize(argv[2]);
    // }
    if (0 == strcmp(argv[1], "enhencer")){
        printf("you are running enhencer!!!!");
        run_enhencer(argc, argv);
    } else if (0 == strcmp(argv[1], "im_resize")){
        printf("you are running im-resize\n");
        resize_image_save(argv[2], 512, 512, argv[3]);
    } else if (0 == strcmp(argv[1], "enhence")){
        printf("you are running enhence!!!\n");
        float thresh = find_float_arg(argc, argv, "-thresh", .5);
        char *filename = (argc > 4) ? argv[4]: 0;
        char *outfile = find_char_arg(argc, argv, "-out", 0);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        test_enhencer("cfg/voc.data", argv[2], argv[3], filename, thresh, .5, outfile, fullscreen);
    }
    else {
